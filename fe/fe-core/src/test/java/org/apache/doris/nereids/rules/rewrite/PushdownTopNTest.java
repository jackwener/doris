...
class PushdownTopNTest extends TestWithFeService implements MemoPatternMatchSupported {
    ...
    void testJoin() {
        ...
        PlanChecker.from(connectContext, plan)
                .applyTopDown(new PushdownTopN())
                .matches(
                        logicalJoin(
                                logicalLimit().when(l -> l.getLimit() == 10 && l.getOffset() == 0),
                                logicalOlapScan()
                        )
                );
    }
